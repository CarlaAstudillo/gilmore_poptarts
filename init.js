...
//Create frost line and frostline legend data //
...
d3.json("gilmore_episodes.json", function(data) {
 ...
 var minimdb = d3.min(data, function(d) {
  return d.imdb_score;
 })

 var maximdb = d3.max(data, function(d) {
  return d.imdb_score;
 })

 //Find row number and change height css according to it

 var rows = Math.floor(data.length / 5) * 264

 d3.select('#poptarts svg').style('height', rows + 'px')
 ...
 var frost = svg.selectAll('svg')
  ...
  .style("fill", function(d) {
   return frostscale(d.imdb_score);
  })
  .style("opacity", 0.9)
  .attr('transform', function(d, i) {
   var x = (i % 5) * poptartSize * 1.25;
